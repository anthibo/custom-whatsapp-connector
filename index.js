...
// webhook listening for waba messages
...
router.post("/cmp-webhook", async (req, res) => {
  // Parse the request body from the POST
  let project_id = req.body.projectId;
  winston.verbose("(wab) Message received from CMP");
  ...
  if (
    ...
    */
    ...
    let firstname = req.body.contact.name
    ...
    if (
      ...
      console.log('Media not supported now')
      return res.sendStatus(200);
      let media;
...
router.post("/start-bot", async (req, res) => {
  try {
    console.log("startBot body:" + JSON.stringify(req.body));
    const code = req.body.code;
    const key = "bottest:" + code.substring(3);
    ...
    if (!value) {
      winston.verbose("No info found on redis for start-bot code. Exit..");
      console.log("test bot code cached value: ", value);
      return res.status(500).send({ message: "failed", err: "No info found on redis for start-bot code" });
    }

    winston.verbose("test bot code cached value: ", JSON.stringify(value));
    console.log("test bot code cached value: ", JSON.stringify(value));

    const parsed_value = JSON.parse(value);
    const project_id = parsed_value.project_id;
    const bot_id = parsed_value.bot_id;

    const CONTENT_KEY = "whatsapp-" + project_id;
    console.log("CONTENT_KEY:", CONTENT_KEY);

    const settings = await db.get(CONTENT_KEY);
    console.log("StartBot Settings: ", JSON.stringify(settings));
    ...
    const result = await bottester.startBotConversationCMP(req.body, bot_id);
    ...
    return res.status(500).send({ message: "failed", err: err.toString() });
  }
});
